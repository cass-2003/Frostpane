...
- ✅ 双击打开、管理员运行、打开文件位置
- ✅ 右键菜单（快捷操作 + 原生 Shell IContextMenu）
- ✅ 拖拽移动 + 网格吸附 + 位置持久化
- ✅ **视觉分区（Fence）** — 毛玻璃栅栏，拖入/拖出图标，移动/缩放/折叠/重命名/emoji图标
- ✅ 系统托盘图标 — Show/Hide、隐藏真实桌面图标、Exit
- ✅ 关闭窗口 → 最小化到托盘
- ✅ 双击桌面空白 → 隐藏窗口（露出真实桌面）
- ✅ 桌面右键菜单 — 新建分区、展开/折叠全部、排序、设置
- ✅ 设置面板 — 开机自启开关
- ✅ 分区/图标布局 JSON 持久化
